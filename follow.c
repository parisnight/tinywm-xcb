...
int main (int argc, char **argv)
{
  ...
  while (1) {
    ...
    case XCB_CREATE_NOTIFY:
      ...
      xcb_create_notify_event_t *enter = (xcb_create_notify_event_t *) ev;
	...
	printf ("Mouse entered window %" PRIu32 ", at coordinates (%" PRIi16
		",%" PRIi16 ")\n", enter->event, enter->event_x,
		enter->event_y);
	xcb_set_input_focus (dpy, XCB_INPUT_FOCUS_POINTER_ROOT, enter->event,
			     XCB_CURRENT_TIME);
	break;
      }
	...
	values[0] =
	  (pointer->root_x + geom->width >
	   screen->width_in_pixels) ? (screen->width_in_pixels -
				       geom->width) : pointer->root_x;
	values[1] =
	  (pointer->root_y + geom->height >
	   screen->height_in_pixels) ? (screen->height_in_pixels -
					geom->height) : pointer->root_y;
